...
/**
 * Utility class for managing JSON files. Provides methods to read, retrieve,
 * and update values in JSON files.
 */
public class JsonFileManager {
	...
	public static String generateStringPayload(String filePath) {
		try {
			...
			LoggerManager.get().error("Error reading JSON file:{} {}", filePath, e.getMessage());
	 ...
	 * @return the value associated with the key as a string, or an empty string if
	...
	public static String getJsonData(String filePath, String keyName) {
		...
		if (!data.isEmpty()) {
			try {
				...
				LoggerManager.get().error("Error parsing JSON data from file: {}{}", filePath, e.getMessage());
			}
		}
		return ""; // Return an empty string if an error occurs
	}
	...
	/**
	 * Updates the value associated with the specified key in the JSON file and
	 * saves the file.
	 *
	 * @param filePath the path to the JSON file.
	...
	public static void setJsonData(String filePath, String keyName, String value) {
		...
		if (!data.isEmpty()) {
			try {
				...
				LoggerManager.get().info("Updated {} in JSON file: {}", keyName, filePath);
			} catch (IOException e) {
				LoggerManager.get().error("Error writing to JSON file:{} {}", filePath, e.getMessage());
			}
		}
	}
}
